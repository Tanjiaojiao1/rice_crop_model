...
def Photoperiod(Pdaily, DLBase, DLOptimum, DLCeiling, headingdate, today):
    ...
    return Photo
def photo_period_based_on_yin(mu=-15.46,zeta=2.06,ep=2.48,dl=10):
    # '''
...
def read_station_weather(station,start,end):
    df=pd.read_table("../data/Meteo(48 sta)/"+str(station)+".txt",encoding='gbk',sep=' * ',engine='python',skiprows=[1])
...
def Trial_Sim():
    ...
    '''
    sun=Sun.Sun()
    df = pd.read_csv('../data/obser_pheno_catalog.csv', encoding="GBK",parse_dates=['reviving data','tillering data','jointing data','booting data','heading data','maturity data'])
    print (df.columns)
    ...
    for ind,row in df.iterrows():
        ...
        dfw['dayL']=dfw.Date.apply(lambda x:sun.dayCivilTwilightLength(year=x.year, month=x.month, day=x.day, lon=row.lon, lat=row.lat))
        dfw['photo_raw']=dfw.dayL.apply(lambda x:photo_period_based_on_yin(dl=x))
        dfw['photo']=dfw.apply(lambda rowt:photo_effect_correct(today=rowt.Date,revd=row['reviving data'],jd=row['jointing data'],hd=row['heading data'],photo=rowt.photo_raw),axis=1)
    ...
    dfall=dfall.set_index(['station ID','Date']).join(dfm[['station ID','Date','DStage']].set_index(['station ID','Date'])).reset_index()
