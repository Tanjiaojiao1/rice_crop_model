...
def Simulate_error_days_for_sites(T_fun,Photo_fun,T_fun_para={},Photo_fun_para={},dflocations:pd.DataFrame=None):
    '''
...
if __name__=="__main__":
    df = pd.read_csv('../data/obser_pheno_catalog.csv', encoding="GBK",
                    parse_dates=['reviving date', 'tillering date', 'jointing date',
                                'booting date', 'heading date','maturity date'])
    Simulate_error_days_for_sites(T_fun=T_base_op_ceiling,Photo_fun=photoeffect_oryza200,dflocations=df)